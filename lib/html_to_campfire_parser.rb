...
#   puts parser.to_campfire
class HTMLToCampfireParser < CampfireSgmlParser
  PRE_SPACE_TOKEN = "~F_O_O~"
  ...
  PAIRS = { 'blockquote' => 'bq', 'p' => 'p', 'pre' => 'pre' }
  QUICKTAGS = { 'b' => '*', 'strong' => '*', 
    'i' => '_', 'em' => '_', 'cite' => '??', 's' => '-', 
    'sup' => '^', 'sub' => '~', 'code' => '\'', 'span' => '%'}
  ...
  def start_pre(attrs)
    write("\n<pre>") # we'll modify this later to be a single line
    start_capture("pre")
  end
  ...
  def feed(data)
    ...
    # remove code elements inside of pres
    data.gsub!(/<pre[^>]*>\s*<code[^>]*>\s*/mi, '<pre>')
    data.gsub!(/\s*<\/code>\s*<\/pre>/mi, '</pre>')
    # make each line of pre its own pre
    data.gsub!(/<pre[^>]*>(.*?)<\/pre>/mi) do |m|
      "<pre>#{$1.gsub(/\n+/m, '</pre><pre>')}</pre>"
    end
    data.gsub!(/<pre[^>]*>(\s+)(.*?)<\/pre>/) do |m|
      "<pre>#{PRE_SPACE_TOKEN * $1.size}#{$2}</pre>"
    end
    ...
    data.gsub!(/<blockquote[^>]*>\s*<p[^>]*>\s*/mi, '<blockquote>')
    data.gsub!(/\s*<\/p>\s*<\/blockquote>/mi, '</blockquote>')
    data.gsub!(/<blockquote[^>]*>(.*?)<\/blockquote>/mi) do |m|
      s = $1
      s.gsub!(/<p[^>]*>/, "\n<blockquote>")
      s.gsub!(/<\/p>/, "</blockquote>\n")
  ...
  # Return the campfire messages after processing
  def to_campfire
    post_process(result)
  end
  ...
  def post_process(lines)
    ...
    # output.strip!
#    return lines.join
#    return lines.join.collect do |line|
      line = lines.join
      # line.gsub!(/ +/m, " ")
      # line.gsub!(/\n+/m, "\n")
      # line.strip!
      ...
      # all pre lines will be a single line with \n's
      lines.inject([]) do |acc, line|
        if line =~ /^<pre>/
          line.gsub!(PRE_SPACE_TOKEN, ' ')
          line = "#{line[5..-1]}\n"
          if acc.last.last == "\n"
            acc[-1] = "#{acc.last}#{line}"
          else
